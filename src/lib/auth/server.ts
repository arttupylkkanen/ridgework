...
const PRODUCTION_ORIGINS: string[] = [
  "https://ridgework.org",
  "https://www.ridgework.org",
];
const extraTrustedOrigins = (env("GROK_AUTH_TRUSTED_ORIGINS") ?? "")
  .split(/[\s,]+/)
  .filter(Boolean);
const baseURL = explicitBaseURL ?? {
  // Include loopback hosts so dynamic baseURL resolves for local email/password
  // (not only the preview wildcard).
  allowedHosts: [...previewAllowedHosts, "localhost", "127.0.0.1", "[::1]", "ridgework.org", "www.ridgework.org"],
...
// Always UNION grok.me + preview + custom domain — never replace the list
// when BETTER_AUTH_URL is set (that caused "Invalid origin" on ridgework.org).
